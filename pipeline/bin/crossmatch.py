...
def ps1(s, ra, dec):
    ...
    for ii,match in enumerate(closest_matches):
        ...
        out['sgscore%s' %(ii+1)] = ps_score[objid==int(oid)][0]
        out['distpsnr%s' %(ii+1)] = dist[order][ii]
        try:
...
def legacysurvey(cur, ra, dec):
    ...
    for ii,match in enumerate(matches):
        ...
        out['lsw1'] = abmag(matches[ii][9])
        out['lsw2'] = abmag(matches[ii][10])
        out['lsw3'] = abmag(matches[ii][11])
        out['lsw4'] = abmag(matches[ii][12])
        out['lsgaiag'] = matches[ii][13]
        out['lsgaiap'] = matches[ii][14]
